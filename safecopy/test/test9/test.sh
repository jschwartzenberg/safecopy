...
function test_current() {
	...
	if ! LD_PRELOAD="$preload" $safecopy -R 2 -b 1024 -f 4* -r 4* -X xblocks -o "$testsuite_tmpdir/test1.badblocks" debug "$testsuite_tmpdir/test1.dat" >"$testsuite_tmpdir/test1.out" 2>&1; then
	...
	if ! LD_PRELOAD="$preload" $safecopy -R 2 -b 1024 -f 8* -X xblocks -I "$testsuite_tmpdir/test1.badblocks" -o "$testsuite_tmpdir/test2.badblocks" debug "$testsuite_tmpdir/test2.dat" >"$testsuite_tmpdir/test2.out" 2>&1; then
		testsuite_error "Run of safecopy failed. Output:"
	...
	if ! LD_PRELOAD="$preload" $safecopy -R 2 -b 1024 -f 1* -X xblocks -I "$testsuite_tmpdir/test1.badblocks" -o "$testsuite_tmpdir/test3.badblocks" debug "$testsuite_tmpdir/test3.dat" >"$testsuite_tmpdir/test3.out" 2>&1; then
		testsuite_error "Run of safecopy failed. Output:"
	...
	if ! LD_PRELOAD="$preload" $safecopy -M "MARKBAAD" -R 2 -b 1024 -f 8* -X xblocks -I "$testsuite_tmpdir/test1.badblocks" -o "$testsuite_tmpdir/test4.badblocks" debug "$testsuite_tmpdir/test4.dat" >"$testsuite_tmpdir/test4.out" 2>&1; then
		testsuite_error "Run of safecopy failed. Output:"
	...
	testsuite_debug " Must not touch exclude blocks."
	if ! LD_PRELOAD="$preload" $safecopy -M "MARKBAAD" -R 2 -b 1024 -f 1* -X xblocks -I "$testsuite_tmpdir/test1.badblocks" -o "$testsuite_tmpdir/test5.badblocks" debug "$testsuite_tmpdir/test5.dat" >"$testsuite_tmpdir/test5.out" 2>&1; then
		testsuite_error "Run of safecopy failed. Output:"
