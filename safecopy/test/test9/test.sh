...
LD_PRELOAD="$safecopydebug" $safecopy -R 2 -b 1024 -f 4* -r 4* -o "$tmpdir/test1.badblocks" debug "$tmpdir/test1.dat" -X xblocks >/dev/null 2>&1
...
LD_PRELOAD="$safecopydebug" $safecopy -R 2 -b 1024 -f 8* -o "$tmpdir/test2.badblocks" debug "$tmpdir/test2.dat" -I "$tmpdir/test1.badblocks" -X xblocks >/dev/null 2>&1

# second incremental run with narrow skipsize. must successfully read the "in between" good block the first run missed, too
LD_PRELOAD="$safecopydebug" $safecopy -R 2 -b 1024 -f 1* -o "$tmpdir/test3.badblocks" debug "$tmpdir/test3.dat" -I "$tmpdir/test1.badblocks" -X xblocks >/dev/null 2>&1

# same again but with badblock marking. must mark all blocks that remain zero in the previous output files but not more
LD_PRELOAD="$safecopydebug" $safecopy -R 2 -b 1024 -f 8* -o "$tmpdir/test4.badblocks" debug "$tmpdir/test4.dat" -I "$tmpdir/test1.badblocks" -M "MARKBAAD" -X xblocks >/dev/null 2>&1
LD_PRELOAD="$safecopydebug" $safecopy -R 2 -b 1024 -f 1* -o "$tmpdir/test5.badblocks" debug "$tmpdir/test5.dat" -I "$tmpdir/test1.badblocks" -M "MARKBAAD" -X xblocks >/dev/null 2>&1
