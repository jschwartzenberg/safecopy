...
//list of filedesciptors
static int descriptorlist[257]={-1};
...
// very simple filedescriptor list
static int in_descriptors(int fd) {
	int t;
	for (t=0;t<descriptorcount;t++) {
		if (descriptorlist[t]==fd) return t;
	}
	return -1;
}
#define is_in_descriptors(fd) ((fd)!=-1 && in_descriptors(fd)>-1)
static void rem_descriptor(int fd) {
	int t;
	for (t=in_descriptors(fd);t<descriptorcount;t++) {
		descriptorlist[t]=descriptorlist[t+1];
	}
	descriptorcount-=1;
}
#define add_descriptor(fd) descriptorlist[descriptorcount++] = fd
...
static int myopen64(const char *pathname,int flags,...) {
	myinit();
	//va_list ap;
	int fd;
	int mode=0;
...
static off64_t mylseek64(int filedes, off64_t offset, int whence) {
	myinit();
...
static int mydup2(int fd,int wish) {
	myinit();
...
int dup2(int fd,int wish) {
	return mydup2(fd,wish);
}
